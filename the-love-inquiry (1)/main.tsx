...
import { Heart, Stars, HeartHandshake, Volume2, VolumeX, Music, LockOpen, Sparkles, PartyPopper } from 'lucide-react';
      ...
      <div className="relative z-10 text-center space-y-6 animate-in fade-in zoom-in duration-700 px-4">
        ...
        {isMusicPlaying && (
          <div className="flex flex-col items-center justify-center gap-2 text-pink-100 text-base animate-pulse font-medium">
            ...
            <span className="text-sm uppercase tracking-widest font-bold italic bg-black/10 px-4 py-1 rounded-full">"Tere Bina Na Guzara Re" & "I Really Really Like You"</span>
          </div>
        )}
        <div className="pt-6">
...
const App: React.FC = () => {
  ...
  const startExperience = () => {
    setIsStarted(true);
    if (audioRef.current) {
      audioRef.current.play().then(() => setIsPlaying(true)).catch(err => console.error("Audio play failed:", err));
    }
  };
  ...
  useEffect(() => {
    if (hasAccepted && audioRef.current) {
      ...
      if (wasPlaying) audioRef.current.play().catch(e => console.log("Success audio blocked:", e));
    }
  ...
  if (!isStarted) {
    return (
      <div className="relative min-h-screen w-full flex flex-col items-center justify-center bg-pink-100 overflow-hidden">
        <FloatingHearts />
        <div className="z-10 text-center space-y-8 p-8 bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl border-4 border-white">
          ...
          <button onClick={startExperience} className="group relative flex items-center gap-3 px-10 py-5 bg-pink-500 text-white font-bold rounded-full hover:bg-pink-600 transition-all shadow-xl hover:scale-105 active:scale-95">
    ...
    <div className="relative min-h-screen w-full flex flex-col items-center justify-center p-4 overflow-hidden bg-gradient-to-br from-pink-100 via-white to-red-50">
      ...
      <div className="z-10 text-center space-y-12 max-w-lg w-full">
        <div className="space-y-6">
          ...
          <h1 className="text-6xl md:text-8xl font-cursive text-pink-600 drop-shadow-sm">Do you love me?</h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-12 min-h-[160px]">
          ...
          <ElusiveButton onAttempt={() => { setNoAttemptCount(prev => prev + 1); setYesButtonScale(prev => Math.min(prev + 0.15, 3)); }} />
        </div>
      </div>
      <footer className="absolute bottom-6 text-xs text-pink-300 font-medium uppercase tracking-widest">Made for my favorite person</footer>
    </div>
  );
};
...
if (root) {
  // @ts-ignore
  const rootNode = (window as any).ReactDOM.createRoot(root);
  rootNode.render(<App />);
}
